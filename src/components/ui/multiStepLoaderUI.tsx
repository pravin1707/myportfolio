...
const LoaderCore = ({
  ...
  return (
    <div className="flex relative justify-start max-w-xl mx-auto flex-col mt-40 bg-transparent">
...
export const MultiStepLoader = ({
  ...
  const [isFinished, setIsFinished] = useState(false); // ADDED: State to prevent multiple onComplete calls
    ...
    const timeout = setTimeout(() => {
      setCurrentState((prevState) => {
        if (prevState === loadingStates.length - 1) {
          return loop ? 0 : prevState; // If not looping, stay at the last state
        }
        return prevState + 1;
      });
    }, duration);
  ...
  }, [currentState, loop, isFinished, onComplete, loadingStates.length]);
    ...
    <AnimatePresence mode="wait">
      {loading && (
        ...
        >
          ...
          <div className="bg-gradient-to-t inset-x-0 z-20 bottom-0 h-full absolute" />
        </motion.div>
      )}
    </AnimatePresence>
  );
